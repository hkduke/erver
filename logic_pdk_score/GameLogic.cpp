...
const CT_BYTE CGameLogic::m_byGoodCardData[GOOD_CARD_COUTN]=
{
    0x32,
    0x01,
    0x11,
...
{
	m_cbNotSplitRomb = 0;
	m_cbCanFourTakeTwo = 1;
	m_cbCanFourTakeThree = 1;
	m_cbIsOutThreeEnd = 0;
	m_cbIsThreeEnd = 0;
	m_cbIsOutFlyEnd = 0;
	m_cbIsFlyEnd = 0;
...
//得到好牌
CT_VOID CGameLogic::GetGoodCardData(CT_BYTE cbGoodCardData[], CT_BYTE cbGoodCardCount)
{
	if(cbGoodCardCount > CountArray(m_byGoodCardData))
	{
		LOG(ERROR) << "cbGoodCardCount ERROR!";
		return;
	}
	//混乱准备
	CT_BYTE cbCardData[CountArray(m_byGoodCardData)];
	CT_BYTE cbCardBuffer[CountArray(m_byGoodCardData)];
	memcpy(cbCardData,m_byGoodCardData,CountArray(m_byGoodCardData));

	//混乱扑克
	CT_BYTE cbRandCount=0,cbPosition=0;
	CT_BYTE cbBufferCount=cbGoodCardCount;
	do
	{
	...
	//复制好牌
	memcpy(cbGoodCardData, cbCardBuffer, cbGoodCardCount);
}
//删除好牌
CT_BOOL CGameLogic::RemoveGoodCardData(CT_BYTE cbGoodCardData[], CT_BYTE cbGoodCardCount, CT_BYTE cbCardData[], CT_BYTE cbCardCount)
{
	//检验数据
	...
	//定义变量
	CT_BYTE cbDeleteCount=0,cbTempCardData[FULL_COUNT];
	if (cbCardCount>CountArray(cbTempCardData)) return false;
	memcpy(cbTempCardData,cbCardData,cbCardCount*sizeof(cbCardData[0]));
	memset(cbCardData,0,cbCardCount*sizeof(cbCardData[0]));
	//置零扑克
	for (CT_BYTE i=0;i<cbGoodCardCount;i++)
	{
...
//删除扑克
CT_BOOL CGameLogic::RemoveCardList(const CT_BYTE cbRemoveCard[], CT_BYTE cbRemoveCount, CT_BYTE cbCardData[], CT_BYTE cbCardCount)
{
	//检验数据
	assert(cbRemoveCount <= cbCardCount);
...
CT_BYTE CGameLogic::GetCardType(const CT_BYTE cbCardData[], CT_BYTE cbCardCount, CT_BOOL bLastCard /*= false*/, CT_BYTE cbAirLineCount[] /*= NULL*/)
{
    ...
    //飞机,拆炸弹组成飞机
    if (m_cbNotSplitRomb != 1 && (AnalyseResult.cbBlockCount[3] == 1) && (cbCardCount >= 8))
    {
        ...
        if (cbLineCount >= 2)
        {
            ...
            if (cbCardCount == 10 || cbCardCount == 8)
            {
            ...
            if (bAirPlaneFalg && cbCardCount==((cbLineCount*3)+(cbLineCount*2)))
            {
                //拆开炸弹作飞机
                return CT_AIRPLANE;
            }
            else if (bAirPlaneFalg && cbCardCount==((cbLineCount*3)+(cbLineCount*1)))
            {
                //拆开炸弹作飞机
                return CT_THREE_LINE_ONE;
            }
            else if (bAirPlaneFalg && cbCardCount==((cbLineCount*3)+(cbLineCount*0)))
            {
    ...
    if (AnalyseResult.cbBlockCount[3]>0)
    {
        ...
        if (AnalyseResult.cbBlockCount[3] >= 2)
        {
            ...
            {
                //变量定义
                CT_BYTE cbCardData = AnalyseResult.cbCardData[3][cbPos * 4];
                ...
                CT_BYTE cbCount = 1;
                CT_BYTE i = cbPos + 1;
                for (/*CT_BYTE i = cbPos+1*/; i < AnalyseResult.cbBlockCount[3]; i++)
                {
                    cbPos = i;
                    CT_BYTE cbCardData = AnalyseResult.cbCardData[3][i * 4];
                ...
                //if (cbCount * 3 == cbCardCount)  return CT_THREE_LINE;
                //if (cbCount * 4 == cbCardCount)  return CT_THREE_LINE_ONE;
                //if (cbCount * 5 == cbCardCount) return CT_AIRPLANE;
                //if (bLastCard && cbCount * 5 > cbCardCount)  return CT_AIRPLANE;  //最后一手牌可以不带


                if ((cbCardCount % 4) == 0 && cbCount >= 2 && (cbCount*4) == cbCardCount)	return CT_FOUR_LINE;
                if ((cbCardCount % 6) == 0 && cbCount >= 2 && (cbCount*6) == cbCardCount)	return CT_FOUR_LINE_TWO;
                if ((cbCardCount % 7) == 0 && cbCount >= 2 && (cbCount*7) == cbCardCount)	return CT_FOUR_LINE_THREE;
                if (cbCount < 2 && i >= AnalyseResult.cbBlockCount[3])
                {
                    break;
                }
            }
        ...
        {
            if (4 == cbCardCount)  return CT_BOMB_CARD;
            if (5 == cbCardCount)  return CT_FOUR_TAKE_ONE;
            if (6 == cbCardCount) return CT_FOUR_TAKE_TWO;
            if (7 == cbCardCount) return CT_FOUR_TAKE_THREE;
        }
        return CT_ERROR;
    }
	...
	if (AnalyseResult.cbBlockCount[2]>0)
	{
		...
		if (AnalyseResult.cbBlockCount[2] >= 2)
		{
			...
			for (CT_BYTE n = 0; n < 10; ++n)
			{
				...
				if ((cbCardCount % 3) == 0 && cbCount >= 2 && (cbCount*3) == cbCardCount)	return CT_THREE_LINE;
				if ((cbCardCount % 4) == 0 && cbCount >= 2 && (cbCount*4) == cbCardCount)	return CT_THREE_LINE_ONE;
				if ((cbCardCount % 5) == 0 && cbCount >= 2 && (cbCount*5) == cbCardCount)	return CT_AIRPLANE;
		...
		{
			if (3 == cbCardCount)  return CT_THREE_EX;
			if (4 == cbCardCount)  return CT_THREE_TAKE_ONE;
...
CT_BOOL CGameLogic::CompareCard(const CT_BYTE cbFirstCard[], CT_BYTE cbFirstCount, const CT_BYTE cbNextCard[], CT_BYTE cbNextCount, CT_BOOL bLastCard/* = false*/)
{
	...
	{
		CT_BYTE cbCount = 3;
		if (cbNextType == CT_FOUR_TAKE_THREE || cbNextType == CT_FOUR_TAKE_TWO || cbNextType == CT_FOUR_TAKE_ONE)
		{
			cbCount = 4;
		}
		//分析扑克
	...
	case CT_THREE_LINE_ONE:
	case CT_AIRPLANE:
    case CT_FOUR_LINE_TWO:
    case CT_FOUR_LINE_THREE:
	{
		CT_BYTE cbSameCount = 3;
		if(CT_FOUR_LINE_TWO == cbNextType|| CT_FOUR_LINE_THREE == cbNextType)
		{
			cbSameCount = 4;
		}
		//搜索最大飞机
		CT_BYTE cbFirstResultCard[MAX_COUNT] = { 0 };
		SearchMaxThreeLine(cbTempFirstCard, cbFirstCount, cbFirstResultCard,cbSameCount);
		CT_BYTE cbNextResultCard[MAX_COUNT] = { 0 };
		SearchMaxThreeLine(cbTempNextCard, cbNextCount, cbNextResultCard,cbSameCount);
...
CT_BYTE CGameLogic::SearchOutCard(const CT_BYTE cbHandCardData[], CT_BYTE cbHandCardCount, const CT_BYTE cbTurnCardData[], CT_BYTE cbTurnCardCount, tagSearchCardResult& SearchCardResult)
{
	...
	case CT_BOMB_CARD:				    //炸弹类型
	{
        ...
        else if (cbTurnOutType == CT_THREE_EX) cbSameCount = 3;
    ...
    case CT_FOUR_LINE:			//三连类型
	{
        ...
        if (cbTurnOutType == CT_THREE_LINE) cbBlockCount = 3;
        if (cbTurnOutType == CT_FOUR_LINE) cbBlockCount = 3;
	...
	{
        CT_BYTE cbSameCount = 3;
        CT_BYTE cbTakeCardCount = 1;
        if (cbTurnOutType == CT_THREE_TAKE_TWO) cbTakeCardCount = 2;
        else if (cbTurnOutType == CT_FOUR_TAKE_ONE) cbSameCount = 4;
        else if (cbTurnOutType == CT_FOUR_TAKE_TWO)
        {
            cbSameCount = 4;
            cbTakeCardCount = 2;

        }
        else if (cbTurnOutType == CT_FOUR_TAKE_THREE)
        {
            cbSameCount = 4;
            cbTakeCardCount = 3;
        }
		...
		{
			cbResultCount = SearchTakeCardType(cbCardData, cbCardCount, cbReferCard, cbSameCount, cbTakeCardCount, SearchCardResult);
		}
		break;
	}
    ...
    case CT_FOUR_LINE_THREE:
	{
        ...
        CT_BYTE cbSameCount = 3;
		if (CT_THREE_LINE_ONE == cbTurnOutType)
        {
            cbLineCount = cbTurnCardCount / 4;
            cbTakeCardCount = 1;
        }
        else if (CT_FOUR_LINE_TWO == cbTurnOutType)
        {
            cbLineCount = cbTurnCardCount / 6;
            cbTakeCardCount = 2;
            cbSameCount = 4;
        }
        else if (CT_FOUR_LINE_THREE == cbTurnOutType)
        {
            cbLineCount = cbTurnCardCount / 7;
            cbTakeCardCount = 3;
            cbSameCount = 4;
        }
        ...
        CT_BYTE cbReferCard = AnalyseResult.cbCardData[2][0];
        if (CT_FOUR_LINE_TWO == cbTurnOutType || CT_FOUR_LINE_THREE == cbTurnOutType)
        {
            cbBlockCount = AnalyseResult.cbBlockCount[3];
            cbReferCard = AnalyseResult.cbCardData[3][0];
        }//4-1链子被当成3带2打
        else if (CT_AIRPLANE == cbTurnOutType && 0 == cbReferCard)
        {
            cbBlockCount = AnalyseResult.cbBlockCount[3];
            cbReferCard = AnalyseResult.cbCardData[3][0];
        }
		...
		{
			//搜索飞机
			cbResultCount = SearchThreeTwoLine(cbCardData, cbCardCount, cbReferCard, cbTakeCardCount, cbLineCount, SearchCardResult,cbSameCount);
		}
		break;
	}
	default:
...
CT_BYTE CGameLogic::SearchHeadOutCard(CT_BYTE cbCardData[], CT_BYTE cbCardCount, tagSearchCardResult& SearchCardResult, CT_BYTE& cbResultCount)
{
   ...
   /* CT_BYTE cbSameCount = 0;
    ...
    {
        memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
        if (SearchSameCard(cbCardData, cbCardCount, 0, 3, tmpSearchCardResult) > 0)
        {//拷贝第一个结果
            CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
        }
    }
    ...
    //三带一
    memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
    if (SearchTakeCardType(cbCardData, cbCardCount, 0, 3, 1, tmpSearchCardResult) > 0)
    {//拷贝第一个结果
        CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
    }
    //三带二
    ...
    //三连
    memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
    if (SearchLineCardType(cbCardData, cbCardCount, 0, 3, 0, tmpSearchCardResult) > 0)
    {//拷贝第一个结果
        CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
    }
    ...
    //四带1
    memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
    if (SearchTakeCardType(cbCardData, cbCardCount, 0, 4, 1, tmpSearchCardResult) > 0)
    {//拷贝第一个结果
        CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
    }
    //四带2
    memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
    if (SearchTakeCardType(cbCardData, cbCardCount, 0, 4, 2, tmpSearchCardResult) > 0)
    {//拷贝第一个结果
        CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
    }
    //四带3
    memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
    if (SearchTakeCardType(cbCardData, cbCardCount, 0, 4, 3, tmpSearchCardResult) > 0)
    {//拷贝第一个结果
        CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
    }
	...
	//飞机带双
	memset(&tmpSearchCardResult, 0, sizeof(tmpSearchCardResult));
	if (SearchThreeTwoLine(cbCardData, cbCardCount, 0, 3, 0, tmpSearchCardResult,4) > 0)
	{//拷贝第一个结果
		CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
	}
	else if (SearchThreeTwoLine(cbCardData, cbCardCount, 0, 2, 0, tmpSearchCardResult,4) > 0)
	{//飞机带单
		CopyFirstResult(SearchCardResult, cbResultCount, tmpSearchCardResult);
	}
...
CT_BYTE CGameLogic::SearchLineCardType(const CT_BYTE cbHandCardData[], CT_BYTE cbHandCardCount, CT_BYTE cbReferCard, CT_BYTE cbBlockCount, CT_BYTE cbLineCount, tagSearchCardResult& SearchCardResult)
{
	...
	for (CT_BYTE cbValueIndex = cbReferIndex; cbValueIndex < 14; cbValueIndex++)
	{
		...
		else
		{
			cbTmpLinkCount++;
			//寻找最长连
			if (cbLineCount == 0 && cbTmpLinkCount<12 && cbValueIndex < 13) continue;
		}
	...
	for (CT_BYTE i = 0; i < cbSameCardResultCount; i++)
	{
		...
		for (CT_BYTE j = 0; j < CountArray(AnalyseResult.cbBlockCount); j++)
		{
			for (CT_BYTE k = 0; k < AnalyseResult.cbBlockCount[j]; k++)
			{
				...
				//过滤相同牌
				if (GetCardValue(SameCardResult.cbResultCard[i][0]) == GetCardValue(AnalyseResult.cbCardData[j][k]))
				{
					continue;
				}
				//防止3带2当有一个单牌且单牌比较大时，把对子拆了
				if ((j==0) && ((AnalyseResult.cbBlockCount[1]*2) >=(cbNeedCount - SameCardResult.cbCardCount[i]))
				&& ((GetLogicValue(AnalyseResult.cbCardData[0][k]) > 13) || ((GetLogicValue(AnalyseResult.cbCardData[0][k]) > GetLogicValue(AnalyseResult.cbCardData[1][0]))
				&& (GetLogicValue(AnalyseResult.cbCardData[1][0]) < 10) && (GetLogicValue(AnalyseResult.cbCardData[0][k]) >= 12))))
				{
					continue;
				}
				//复制带牌
				CT_BYTE cbCount = SameCardResult.cbCardCount[i];
				CT_BYTE cbATempTakeCardCount = (j + 1);
				if (cbCount + cbATempTakeCardCount > cbNeedCount) cbATempTakeCardCount = cbNeedCount - cbCount;
				memcpy(&SameCardResult.cbResultCard[i][cbCount], &AnalyseResult.cbCardData[j][k*(j + 1)], sizeof(CT_BYTE)*(cbATempTakeCardCount));

				SameCardResult.cbCardCount[i] += cbATempTakeCardCount;
...
//搜索飞机
CT_BYTE CGameLogic::SearchThreeTwoLine(const CT_BYTE cbHandCardData[], CT_BYTE cbHandCardCount, CT_BYTE cbReferCard, CT_BYTE cbTakeCardCount, CT_BYTE cbLineCount, tagSearchCardResult& SearchCardResult,CT_BYTE cbSameCount/* = 3*/)
{
	//设置结果
	memset(&SearchCardResult, 0, sizeof(tagSearchCardResult));
	//结果数量
	...
	//搜索连牌
	CT_BYTE cbTmpResultCount = SearchLineCardType(cbHandCardData, cbHandCardCount, cbReferCard, cbSameCount, cbLineCount, tmpSearchResult);
	...
	for (CT_BYTE i = 0; i < cbTmpResultCount; i++)
	{
		...
		//连牌个数
		CT_INT32 iTempLineCount = tmpSearchResult.cbCardCount[i] / cbSameCount;
		//带牌个数
		CT_INT32 iTempTakeCount = iTempLineCount*cbTakeCardCount;
		//除去顺子剩余多少
		CT_BYTE cbRemainCount = cbHandCardCount - iTempLineCount * cbSameCount;
		...
		if (cbRemainCount < iTempTakeCount)
		{
			...
			//拆分连牌
			RemoveCard(tmpSearchResult.cbResultCard[i], (cbCount * cbSameCount), tmpSearchResult.cbResultCard[i], tmpSearchResult.cbCardCount[i]);
			tmpSearchResult.cbCardCount[i] -= (cbCount * cbSameCount);
		}
		...
		//排列扑克
		SortCardList(cbTmpCardData, cbTmpCardCount, ST_ACS);
		...
		{
			for (int k = 0; k < TmpResult.cbBlockCount[j]; ++k) {
				//防止3带2当有一个单牌且单牌比较大时，把对子拆了
				if ((j==0) && ((TmpResult.cbBlockCount[1]*2 + cbDistillCount)>= iTempTakeCount)
					&& (GetLogicValue(TmpResult.cbCardData[0][k]) >= 13) && GetLogicValue(TmpResult.cbCardData[0][k] >= GetLogicValue(TmpResult.cbCardData[1][0])))
				{
					continue;
				}
				//复制带牌
				CT_BYTE cbATempTakeCardCount = (j + 1);
				if (iTempTakeCount < cbDistillCount + cbATempTakeCardCount) cbATempTakeCardCount = iTempTakeCount - cbDistillCount;
				memcpy(&cbDistillCard[cbDistillCount], &TmpResult.cbCardData[j][k*(j + 1)], sizeof(CT_BYTE)*(cbATempTakeCardCount));

				cbDistillCount += cbATempTakeCardCount;

				if (cbDistillCount < iTempTakeCount) continue;
...
//找最大对子
CT_BYTE CGameLogic::SearchMaxDouble(const CT_BYTE *cbHandCardData, CT_BYTE cbHandCardCount, CT_BYTE cbReferCard, CT_BYTE *cbResultCard)
{
	//拷贝牌
	CT_BYTE cbTempCard[MAX_COUNT] = { 0 };
	memset(cbTempCard, 0, sizeof(cbTempCard));
	...
	SearchSameCard(cbTempCard,cbHandCardCount,cbReferCard,2,SearchCardResult);
	//排列扑克(大到小排序)
	int i = 0;
	int index = 0;
	CT_BYTE cbTempLogicValue = GetLogicValue(SearchCardResult.cbResultCard[0][0]);
	for (i = 0; i < SearchCardResult.cbSearchCount; ++i)
	{
		if ((0!= i) && cbTempLogicValue < GetLogicValue(SearchCardResult.cbResultCard[i][0]))
		{
			cbTempLogicValue = GetLogicValue(SearchCardResult.cbResultCard[i][0]);
			index = i;
		}
	}

	if (SearchCardResult.cbSearchCount > 0 && GetLogicValue(SearchCardResult.cbResultCard[index][0]) > GetLogicValue(cbReferCard))
	{
		memcpy(cbResultCard, SearchCardResult.cbResultCard[index], sizeof(CT_BYTE)*SearchCardResult.cbCardCount[index]);
		return 1;
	}
	return 0;
}
...
//搜索最大飞机
CT_BYTE CGameLogic::SearchMaxThreeLine(CT_BYTE cbCardData[], CT_BYTE cbCardCount, CT_BYTE cbResultCardData[],CT_BYTE cbSameCardCount/*=3*/)
{
	//排列扑克(小到大排序)
	SortCardList(cbCardData, cbCardCount, ST_ACS);
	...
	else if (2 == cbLineCount)
    {
        SortCardList(cbLineCard, cbLineCount, ST_ACS);
        CT_BYTE cbLogicValue1 = GetLogicValue(cbLineCard[0]);
        CT_BYTE cbLogicValue2 = GetLogicValue(cbLineCard[1]);
        if ((cbLogicValue2 - cbLogicValue1) == 1)
        {
            cbMaxLogicCard = cbLineCard[1];
        }
    }
...
CT_BYTE CGameLogic::GetRemainCardType(const CT_BYTE cbCardData[], CT_BYTE cbCardCount,const CT_BYTE cbRemoveCardData[], CT_BYTE cbRemoveCardCount, tagAnalyseResult * pAnalyseResult /*= NULL*/, CT_BOOL * bIsLine/* = NULL*/)
{
    CT_BYTE cbTempCardData[MAX_COUNT] = { 0 };
    if (cbCardCount < cbRemoveCardCount) return  CT_ERROR;
    if (cbCardCount > CountArray(cbTempCardData)) return  CT_ERROR;
    memcpy(cbTempCardData, cbCardData, cbCardCount * sizeof(cbCardData[0]));
    //删除扑克
    if (RemoveCardList(cbRemoveCardData, cbRemoveCardCount, cbTempCardData, cbCardCount) == false)
    {
        //assert(FALSE);
    ...
    if (pAnalyseResult)
    {
        AnalysebCardData(cbTempCardData,cbCardCount-cbRemoveCardCount, *pAnalyseResult);
    }
	...
	memset(&tmpSearchResult, 0, sizeof(tagSearchCardResult));
	//搜索连牌
	CT_BYTE cbTmpResultCount = SearchLineCardType(cbTempCardData, cbCardCount-cbRemoveCardCount, 0, 1, 5, tmpSearchResult);
	if (0 < cbTmpResultCount && bIsLine)
	{
		*bIsLine = true;
	}

    return GetCardType(cbTempCardData, cbCardCount-cbRemoveCardCount);
}