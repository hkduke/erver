...
private:
	...
	CT_VOID LoadBrGameChangeCardRatio();
	CT_VOID LoadBrGameBlackList();
	CT_VOID LoadZjhCardDepotRatio();
	CT_VOID LoadStockControlInfo();
	CT_VOID LoadDianKongInfo();
	CT_VOID LoadBlackChannel();
	CT_BOOL ConnectPlatformDB();
    CT_BOOL LoadJSFishSystemInfo(acl::mysql_manager &dbManager, tagJSFishSystemInfo &info);
    CT_BOOL LoadJSFishControlInfo(acl::mysql_manager &dbManager, tagJSFishControlInfo &info);
	//功能函数
private:
	//redis相关
	...
	inline CT_BOOL CheckUserReadyEnterRoom(CT_DWORD dwUserID);
	inline CT_VOID ExpireUserReadyEnterRoom(CT_DWORD dwUserID);
	inline CT_VOID DelUserReadyEnterRoom(CT_DWORD dwUserID);
...
public:
	...
	CT_VOID OnUserEnterGroupPRoom(acl::aio_socket_stream* pSocket, MSG_PG_EnterGroupPRoom* pPrivateRoom);
	...
	CT_VOID OnUserChangeTable(CT_DWORD dwUserID, CT_BOOL bForce = false);
	CT_VOID OnUserReportUser(MSG_CS_Report_User* pReportUser);

public:
    CT_VOID UpdateGroupPRoomUserInfo(CServerUserItem* pUserItem, CT_DWORD dwRoomNum, bool bEnter);
...
public:
	...
	void	BuildDdzAiThread(void);
	void    HandleDdzAiAction();
...
private:
	...
	VecWaitList		m_listWaitList;			//等待上桌玩家列表
