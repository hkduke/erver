...
CT_VOID	CGSGroupMgr::ModifyGroupInfo(MSG_CS_Modify_GroupInfo* pModifyGroupInfo)
{
	...
	//std::unique_ptr<tagGSGroupInfo>& pGroupInfo = it->second->GetGroupInfo();
...
CT_VOID CGSGroupMgr::AddGroupPRoom(MSG_C2GS_Add_GroupPRoom* pGroupPRoom)
{
    auto it = m_mapGroup.find(pGroupPRoom->dwGroupID);
    if (it == m_mapGroup.end())
    {
        return;
    }

    //it->second->AddGroupPRoom(pGroupPRoom->dwRoomNum);

    auto itRoom = m_mapGroupPRoom.find(pGroupPRoom->dwRoomNum);
    if (itRoom != m_mapGroupPRoom.end())
    {
        LOG(WARNING) << "add group proom fail, room num : " << pGroupPRoom->dwRoomNum << ", group id: " << pGroupPRoom->dwGroupID;
        return;
    }

    m_mapGroupPRoom.insert(std::make_pair(pGroupPRoom->dwRoomNum, pGroupPRoom->dwGroupID));
}

CT_VOID CGSGroupMgr::RemoveGroupPRoom(CT_DWORD dwRoomNum)
{
    auto it = m_mapGroupPRoom.find(dwRoomNum);
...
CT_DWORD CGSGroupMgr::GetPRoomGroupID(CT_DWORD dwRoomNum)
{
    auto it = m_mapGroupPRoom.find(dwRoomNum);
    if (it == m_mapGroupPRoom.end())
    {
...
CT_BOOL CGSGroupMgr::CheckCanEnterGroupPRoom(CT_DWORD dwUserID, CT_DWORD dwRoomNum)
{
    auto itRoomNum = m_mapGroupPRoom.find(dwRoomNum);
    if (itRoomNum == m_mapGroupPRoom.end())
    {
        return false;
    }

    CT_DWORD dwGroupID = itRoomNum->second;
    auto it = m_mapGroup.find(dwGroupID);
    if (it == m_mapGroup.end())
    {
        return false;
