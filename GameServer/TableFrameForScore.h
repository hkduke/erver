...
{
public:
	CTableFrameForScore(CGameServerThread*  m_pGsThread);
...
public:
	...
	//获取玩家的帐号基础信息
	virtual GS_UserBaseData* GetUserBaseData(CT_DWORD dwChairID);
    //获取玩家的金币信息
    virtual GS_UserScoreData* GetUserScoreData(CT_DWORD dwChairID);
	//发送邮件
	virtual CT_VOID SendUserMail(tagUserMail* pUserMail);
	//获取玩家椅子号(返回INVALID_CHAIR, 说明不在线)
	virtual CT_WORD GetUserChairID(CT_DWORD dwUserID);
...
public:
	...
	virtual CT_BOOL RecordHongBaoSLInfo(RecordHBSLInfo* pHbInfo, RecordHBSLGrabInfo* pGrabInfo, CT_WORD wUserCount);
	//写回放的空闲场景
	virtual CT_BOOL WriteGameSceneToRecord(CT_VOID* pData, CT_DWORD dwDataSize, CT_WORD wSubMsgID);
	//获取桌子回调类
	...
	virtual CT_VOID NotifySysWinScore(CT_LONGLONG llSysWinScore) {
		CFishControl::get_instance().NotifySysWinScore(llSysWinScore);
	}
	//玩家每次得分就通知系统输了多少分
	virtual CT_VOID NotifySysLoseScore(CT_LONGLONG llSysLoseScore) {
		CFishControl::get_instance().NotifySysLoseScore(llSysLoseScore);
	}
	virtual CT_LONGLONG GetFishKuCun() {
		return CFishControl::get_instance().GetFishKuCun();
	}
	//点控玩家输钱的时候,玩家输分了 
	virtual CT_VOID NotifySysWinScore_DianKong(CT_DWORD dwUserID, CT_LONGLONG llWinScore, CT_LONGLONG llKuCun, CT_LONGLONG llBloodPoolDianKongWinLose){
		CFishControl::get_instance().NotifySysWinScore_DianKong(dwUserID,llWinScore, llKuCun, llBloodPoolDianKongWinLose);
	}
	//点控玩家赢钱的时候,玩家赢分了 
	virtual CT_VOID NotifySysLoseScore_DianKong(CT_DWORD dwUserID, CT_LONGLONG llLoseScore, CT_LONGLONG llKuCun, CT_LONGLONG llBloodPoolDianKongWinLose) {
		CFishControl::get_instance().NotifySysLoseScore_DianKong(dwUserID, llLoseScore, llKuCun, llBloodPoolDianKongWinLose);
	}
	//保存点控玩家数据
	...
	{
		info.gameIndex = m_pRoomKindInfo->wGameID*10000+m_pRoomKindInfo->wKindID*100+m_pRoomKindInfo->wRoomKindID;
		CFishControl::get_instance().TongJi(info);
	};

	//捕鱼统计单个玩家的输赢信息:用于测试
	virtual CT_VOID SinglePlayerMostWinLostScore(CT_DWORD dwUserID , CT_LONGLONG addScore)
	{
		CFishControl::get_instance().SinglePlayerMostWinLoseScore(dwUserID, addScore);
	}
	//玩家点控开始
	virtual CT_VOID UserDianKongStart(CT_DWORD dwUserID) {
		CFishControl::get_instance().UserDianKongStart(dwUserID);
	}
	//玩家点控结束
	virtual CT_VOID UserDianKongEnd(CT_DWORD dwUserID) {
		CFishControl::get_instance().UserDianKongEnd(dwUserID);
	}

	//保存血池信息(新)
	virtual CT_VOID SaveStockControlInfo(tagStockControlInfo* pStockControlInfo);
	//获取捕鱼系统税收
	virtual CT_DWORD GetFishTaxPro() {
		return CFishControl::get_instance().GetFishTaxPro();
	}
...
public:
	...
	//广播用户信息
	CT_VOID BroadcastUserInfo(CServerUserItem* pUser, CT_BOOL bReConnect);
	//广播用户状态
...
public:
    CT_VOID SetPrivateTableInfo(const PrivateTableInfo& privateInfo) {	memcpy(&m_PrivateTableInfo, &privateInfo, sizeof(PrivateTableInfo)); }
    CT_VOID	SetTableLock(CT_BOOL bLock) { m_TableState.bIsLock = bLock; }
	CT_BOOL GetTableisLock() { return m_TableState.bIsLock; }
	CT_DWORD GetRoomNum() { return  m_PrivateTableInfo.dwRoomNum; }
...
public:
	...
	CT_VOID IncrbyUserPlayCount(CT_DWORD dwUserID);
	CT_VOID IncrbyUserTotalWinScore(CT_DWORD dwUserID, CT_INT32 iScore);
...
public:
	...
	//更新玩家金币
	inline CT_VOID UpdateUserScoreToDB(CT_DWORD dwUserID, CT_LONGLONG llAddScore, CT_BOOL bAndroid);
	...
	//更新举报信息
    inline CT_VOID InsertReportInfoToDB();

	//广播玩家游戏赢分信息
	inline CT_VOID SendUserWinScoreInfo(CServerUserItem* pUserItem, const ScoreInfo& ScoreData);
	...
	//功能函数
public:
	CT_BOOL	GetBenefitReward(CServerUserItem* pUserItem, tagBenefitReward* pBenefitReward);
	inline CT_VOID CheckBenefitReward(CServerUserItem* pUserItem, tagBenefitReward* pBenefitReward);
	CT_BOOL IsLetAndroidOffline(CServerUserItem* pUserItem);
	CT_VOID LetAndroidLeaveNoRealPlayerTabel();
	CT_VOID AddEarnScoreInfo(CServerUserItem* pUserItem, CT_LONGLONG llAddEarnScore);
	CT_DWORD GetAndroidLeaveTime() { return m_dwAndroidLeaveTime; }
	CT_VOID InsertUserReportInfo(CServerUserItem* pUser, CT_DWORD dwUserID, CT_DWORD dwBeReportUserID);
	//获取玩家头像索引
	CT_BYTE GetHeadIndexInCache(CT_DWORD dwUserID);
	// 获取玩家的性别
	CT_BYTE GetUserSexInCache(CT_DWORD dwUserID);
	CT_VOID ChangeTable();

	//回放基本流消息
private:
	inline CT_VOID WriteVideoCommonPackage(MSG_GameMsgDownHead* pDownData, CT_DWORD dwLen);
	inline CT_VOID WriteEnterSuccMsg();
	inline CT_VOID WriteUserEnterMsg();

private:
	//定时器列表
	//TimerInfo	        m_SysTimer[DW_SYS_TIMER_NUMS];
	std::vector<TimerInfo> 	m_SysTimer;

private:
	ITableFrameSink*	m_pTableFrameSink;
	TableState			m_TableState;
	PrivateTableInfo	m_PrivateTableInfo;
    CGameServerThread*  m_pGsThread;
	...
	CT_DWORD			m_dwAndroidLeaveTime;

	MSG_SC_AutoGet_Benefit m_benefitReward;
	//std::map<CT_DWORD , std::set<CT_DWORD >>	m_mapReportInfo;
	
	//录像文件数据
	ReadWriteBuffer		m_GameRecord;

	CT_DWORD 		    m_dwTimerPlus;
};