...
bool use_kernel = true;
	...
	LOG(INFO) << "正在启动客服服务器...";
	pSerCfg = new CServerCfg("./script/customerserver.cf");
	pSerCfg->Load();

	pNetModule = new CNetModule(&handle);
	CNetModule::getSingletonPtr()->SetServerType(CUSTOMER_SERVER);
	CNetModule::getSingletonPtr()->InitNetModule<CCustomerThread>(1);
	...
	LOG(WARNING) << "address: " << CServerCfg::m_LocalAddress;
	if (!bCreate)
	{
