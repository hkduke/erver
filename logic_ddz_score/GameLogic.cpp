...
const CT_BYTE	CGameLogic::m_cbGoodCardData[GOOD_CARD_COUTN]=
{
	0x01,0x02,
	0x11,0x12,
	0x21,0x22,
...
#define BOM_COUNT 10		//最多10个炸弹
	...
	CT_BYTE ucBomData[BOM_COUNT][4];	//保存炸弹数据
	memset(ucBomData, 0 , sizeof(ucBomData));
...
{
	if(cbGoodCardCount > CountArray(m_cbGoodCardData))
	{
	...
	//混乱准备
	CT_BYTE cbCardData[CountArray(m_cbGoodCardData)];
	CT_BYTE cbCardBuffer[CountArray(m_cbGoodCardData)];
	memcpy(cbCardData,m_cbGoodCardData,CountArray(m_cbGoodCardData));
...
CT_VOID CGameLogic::AnalysebCardData(const CT_BYTE cbCardData[], CT_BYTE cbCardCount, tagAnalyseResult & AnalyseResult)
{
	...
	for (CT_BYTE i = 0; i < cbCardCount; i++)
	{
		...
		for (CT_BYTE j = i + 1; j < cbCardCount; j++)
		{
			...
			if (cbSameCount >= 4)
			{
				/*LOG(ERROR) << "analyze card error? same card out more than 4?? " << ", card data: ";
				for (int z = 0; z < cbCardCount; ++z)
				{
					LOG(ERROR) << std::hex << (int)cbCardData[z];
				}
				printf("\n");*/
				break;
			}
