...
public:
    ...
    */
     ...
     */
    void SetInternalBuffer(size_t a_nInitialSize, size_t a_nGrowBy = SIZE_1K); // throw std::bad_alloc
    ...
    /*! @brief Write the supplied data to the write buffer. 
        ...
        @throw std::bad_alloc       when using an internal buffer and there was an out
                                    of memory error on resizing.
     */
    void WriteBytes(const char * a_pBuf, size_t a_nBufLen); // throw std::overflow_error, std::bad_alloc
    ...
    */
     ...
     */
    void ReadBytes(char * a_pBuf, size_t a_nBufLen); // throw std::invalid_argument
...
private:
    size_t mBufSiz; //!< max buf size,缓冲大小
    size_t mBufLen; //!< committed writes,写位置.
    size_t mBufIdx; //!< committed reads,读位置
    size_t mGrowBy; //!< growby rate,扩展字节数.
    bool   mIsExt;  //!< is this an external buffer?,是否需要扩展.
    char * mBuf;    //!< buffer,缓冲指针
};
