...
public:
	...
	static std::string floatToString(CT_FLOAT val);
	static std::string uintToString(CT_UINT32 val);
	static std::string intToString(CT_INT32 val);
	static std::string boolToString(CT_BOOL val);
	static CT_FLOAT stringToFloat(const std::string& str);
	static CT_UINT32 stringToUint(const std::string& str);
	static CT_UINT64 stingToUint64(const std::string& str);
	static CT_INT32 stringToInt(const std::string& str);
	static CT_BOOL stringToBool(const std::string& str);
	static CT_BOOL stringSplit(const std::string& src, const std::string delim, std::vector<std::string>& ret);
	static double round(double number, unsigned int bits);
	static CT_BOOL isDigits(const std::string& strNum);
	...
	//ip2int
	static unsigned int ip2Int(const std::string & strIP);
	static std::string Int2StringIp(unsigned int nIpNum);
	...
	static std::shared_ptr<ISocketAddress> CreateAddress(struct sockaddr *,socklen_t);
	...
	/* ansi2utf8 */
	static const std::string Ansi2Utf8(const std::string& strAnsi);
	/* utf82ansi */
	static const std::string Utf82Ansi(const std::string& strUtf8);
	...
	static int Utf2Gbk(const char *inbuf, size_t inlen, char *outbuf, size_t outlen)
	{
		return CodeConvert("utf-8", "gb2312", inbuf, inlen, outbuf, outlen);
	}

	static int Gbk2Utf(const char *inbuf, size_t inlen, char *outbuf, size_t outlen)
	{
	...
	static std::string urlEncodeCPP(std::string s);
	static std::string urlDecodeCPP(std::string& s);
	...
	//get int64 rand
	static unsigned long long ullrand(void);

	//get now time
	static unsigned long GetTime();

	//get now time string
	static std::string GetTimeNowString(const std::string& format = DEFAULT_TIME_FORMAT);

	static std::string YesterdayString();

	static std::string ChangeTimeToString(CT_DWORD dwTime, const std::string& format = DEFAULT_TIME_FORMAT);

	static std::string GetNextYear(const std::string& thisYear);

	static std::string GetNextMonth(const std::string& thisMonth);

	static std::string GetLastYearMonth(const std::string& date);

	static int getLocalTime(struct tm* _Tm, const CT_UINT64* _Time);
	static int getLocalTime(struct tm* _Tm, CT_UINT64 _Time);

	static bool isAcrossTheDay(time_t _early, time_t _late);

	static bool IsMobileNum(std::string& mobileNum);
