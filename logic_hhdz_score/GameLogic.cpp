...
//////////////////////////////////////////////////////////////////////////////////
//扑克数据
CT_BYTE CGameLogic::m_cbCardListData[MAX_CARD_TOTAL] =
{
...
void CGameLogic::RandCardList(CT_BYTE cbCardBuffer[], CT_BYTE cbBufferCount)
{
	...
	do
	{
		...
		std::uniform_int_distribution<> dis(0, CountArray(m_cbCardListData) - bRandCount - 1);
		bPosition = dis(gen);
...
//对比扑克
CT_BYTE CGameLogic::CompareCard(CT_BYTE cbFirstData[], CT_BYTE cbNextData[], CT_BYTE cbCardCount)
{
	//设置变量
...
//获取牌型倍数
CT_BYTE	CGameLogic::GetCardMultiple(CT_BYTE cbCardType, CT_BYTE cbCardData[])
{
	switch (cbCardType)
	{
	case CT_BAO_ZI:
	...
	{
		//对牌分析
		CT_BYTE bDouble = false;
		CT_BYTE cbCardLogicValue = 0;

		for (CT_BYTE i = 0; i < MAX_COUNT - 1; i++)
		{
			for (CT_BYTE j = i + 1; j < MAX_COUNT; j++)
			{
				CT_BYTE cbCardLogicValue1 = GetCardLogicValue(cbCardData[i]);
				CT_BYTE cbCardLogicValue2 = GetCardLogicValue(cbCardData[j]);
				if (cbCardLogicValue1 == cbCardLogicValue2)
				{
					bDouble = true;
					cbCardLogicValue = cbCardLogicValue1;
					break;
				}
			}
			if (bDouble)break;
