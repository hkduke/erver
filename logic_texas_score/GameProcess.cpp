...
void CGameProcess::OnEventGameStart()
{
    ...
    */
	...
	//托管
	IsTrustee(GameStart.cbTimeLeft);
	return;
}
...
void CGameProcess::OnTimerMessage(CT_DWORD dwTimerID, CT_DWORD dwParam)
{
	...
	{
        m_pGameDeskPtr->SetGameTimer(IDI_OUT_CARD, 45000);
...
CT_BOOL CGameProcess::OnUserPassCard(CT_WORD wChairID)
{
	...
	if ((m_dwBombChairID != INVALID_CHAIR) && (m_dwTurnWiner == m_dwBombChairID) && (m_dwCurrentUser == m_dwBombChairID))
	{
	    //炸弹结算
...
CT_BOOL CGameProcess::IsTrustee(CT_DWORD dwTime)
{
	...
	case GAME_SCENE_PLAY:
	{
		...
		if (m_cbOpTotalTime<(time(NULL)-m_dwOpStartTime))
        {
            m_pGameDeskPtr->KillGameTimer(IDI_OUT_CARD);
            OnAutoOutCard();
        }
		...
		{
			m_pGameDeskPtr->SetGameTimer(IDI_OUT_CARD, ((TIME_TRUSTEE > dwTime && dwTime != 0) ? dwTime: TIME_TRUSTEE)*1000);
		}
		else if (dwTime==1)
		{
		...
		{
			m_pGameDeskPtr->SetGameTimer(IDI_OUT_CARD, ((dwTime + TIME_NETWORK_COMPENSATE) * 1000));
			//LOG(WARNING) << "SET IDI_OUT_CARD TIMER m_dwCurrentUser: "<< m_dwCurrentUser;
		}
	}
	break;
