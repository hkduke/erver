...
//获取手牌类型
BYTE CGameLogic::GetHandCardType(BYTE cbCardData[], BYTE cbCardCount)
{
...
{
	if (cbCenterCardCount > MAX_CENTERCOUNT || cbCenterCardCount < 3)
	{
		log_error("cbCenterCardCount %d", cbCenterCardCount);
		return 0;
	}
	//临时变量
	...
	CopyMemory(&cbTempCardData[2], cbCenterCardData, cbCenterCardCount);

	//排列扑克
	SortCardList(cbTempCardData, (MAX_COUNT + cbCenterCardCount));
	...
	for (int i=0;i<3;i++)
	{
		if (0 == cbTempCardData[i]) {continue;}
		for (int j= i+1;j<4;j++)
		{
			if (0 == cbTempCardData[j]) {continue;}
			for (int k = j+1;k<5;k++)
			{
				if (0 == cbTempCardData[k]) {continue;}
				for (int l =k+1;l<6;l++)
				{
					if (0 == cbTempCardData[l]) {continue;}
					for (int m=l+1;m<7;m++)
					{
						if (0 == cbTempCardData[m]) {continue;}
						//获取数据
...
bool CGameLogic::SelectMaxUser(BYTE bCardData[GAME_PLAYER][MAX_CENTERCOUNT], UserWinList &EndResult)
{
	//清理数据
...
//查找最大
bool CGameLogic::SelectMaxUser(BYTE bCardData[GAME_PLAYER][MAX_CENTERCOUNT],UserWinList &EndResult,const int64_t lAddScore[])
{
	//清理数据
	ZeroMemory(&EndResult,sizeof(EndResult));
	...
	if(EndResult.bSameCount>1 && lAddScore!=NULL)
	{
		...
		while((iSameCount--)>0)
		{
			int64_t lTempSocre = lAddScore[EndResult.wWinerList[iSameCount]];
	...
	case CT_HU_LU:			//葫芦
		{
			...
			if (AnalyseResultFirst.cbFourCount>0)
			{
				...
				//比较单牌
				if (!(AnalyseResultFirst.cbSignedCount == 1 && AnalyseResultNext.cbSignedCount == 1))
				{
					log_debug("分析扑克错误");
				}
			...
			if (AnalyseResultFirst.cbThreeCount>0)
			{
				...
				{
					//比较对牌
					if (!(AnalyseResultFirst.cbLONGCount == 1 && AnalyseResultNext.cbLONGCount == 1))
					{
						log_debug("判断牌型错误");
					}
				...
				{
					//比较单牌
					if (AnalyseResultFirst.cbSignedCount != 2 || AnalyseResultNext.cbSignedCount != 2)
					{
						log_debug("分析扑克错误2");
					}
			...
			//比较单牌
			if( i == AnalyseResultFirst.cbLONGCount)
			{
				if (!(AnalyseResultFirst.cbSignedCount == AnalyseResultNext.cbSignedCount
