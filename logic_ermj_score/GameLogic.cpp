...
//有效判断
    CT_BOOL CGameLogic::IsValidCard(CT_BYTE cbCardData)
    {
        ...
        return (((cbValue>=1)&&(cbValue<=9)&&(cbColor==0))||((cbValue>=1)&&(cbValue<=7)&&(cbColor==3)));
    }
...
//分析扑克
                                   ...
                                   const CAnalyseItemArray& AnalyseItemArray, CT_WORD& wHuMultiple, tagAnalyseItem& LastAnalyseItem, CT_BOOL& cbIsZiMo,CT_BOOL bCheckTing /*= CT_FALSE*/)
    {
        ...
        //CT_BOOL bJiang258 = Check258Jiang(AnalyseItemArray);
        CT_BOOL bJiang258 = CT_FALSE;
        //CT_BOOL bYaoJiuTou = CheckYaoJiuTou(AnalyseItemArray);
       //CT_BOOL bYaoJiuTou = CT_FALSE;
        cbIsZiMo = (!bCheckTing && wProvideUser==wChairID);

        for (CT_BYTE i = 0; i < AnalyseItemArray.GetCount(); ++i)
        {
            if (AnalyseItemArray[i].cbIsQiDui)
            {
                memcpy(cbCardIndex, cbHandCardIndex[wChairID], sizeof(cbCardIndex));
            }
            else if (MAX_COUNT > GetHandcardIndex(AnalyseItemArray[i], cbCardIndex))
            {
                assert(CT_FALSE);
                continue;
            }
            ...
            //二人麻将打完就重新匹配没有圈的概率，没有圈风刻
            /*if (!AnalyseItemArray[i].cbIsQiDui && !eFlag.TestFlag<HU_TYPE_DA_SI_XI>() && CheckQuanFengKe(AnalyseItemArray[i], cbQuanFeng))
            {
                eFlag.SetFlag<HU_TYPE_QUAN_FENG_KE>();
                wMultiple += 2;

                //str.AppendFormat(_T("圈风刻 "));
               // LOG(ERROR) << "能胡:圈风刻";
            }*/

            //二人麻将也没有门风刻
            /*if (!AnalyseItemArray[i].cbIsQiDui && !eFlag.TestFlag<HU_TYPE_DA_SI_XI>() && CheckMenFengKe(AnalyseItemArray[i], cbMenFeng))
            {
                eFlag.SetFlag<HU_TYPE_MEN_FENG_KE>();
                wMultiple += 2;

                //str.AppendFormat(_T("门风刻 "));
                //LOG(ERROR) << "能胡:门风刻";
            }*/

            //if (!AnalyseItemArray[i].cbIsQiDui && !bQingYiSe && !eFlag.TestFlag<HU_TYPE_YI_SE_SHUANG_LONG>() && CheckPingHu(AnalyseItemArray[i]))
            //if (!AnalyseItemArray[i].cbIsQiDui && !eFlag.TestFlag<HU_TYPE_YI_SE_SHUANG_LONG>() && CheckPingHu(AnalyseItemArray[i]))
            ...
            /*CString FunctionName;
            GetFunctionName;
            //str.AppendFormat(_T("番数:%d 上组番:%d {%s}"), wMultiple, wHuMultiple, FunctionName);
            if (!bCheckTing)
            {
                PrintLog(str, szGameRoomName);
            }*/
