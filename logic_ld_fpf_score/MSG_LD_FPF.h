...
#define GAME_NAME					TEXT("跑胡子-娄底放炮罚")							//游戏名字
...
struct  FPF_CMD_S_GameEnd
{
	...
	//CT_LONG									lGameTax;								//游戏税收(娱乐场用)
	//CT_LONG									lGameGold[GAME_PLAYER];					//游戏积分(娱乐场用)
	CT_BYTE									cbTimeLeave;									//剩余时间
};
...
struct FPF_CMD_S_CLEARING
{
	...
	CT_INT32								iTotalHuXi[GAME_PLAYER];						//各玩家累计胡息
	CT_DOUBLE								dTotalLWScore[GAME_PLAYER];						//各玩家总输赢分
