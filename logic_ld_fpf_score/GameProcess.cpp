...
#define ID_TIME_OFFLINE						    (105)				//离线定时器

#define TIME_READY								(15)				//准备
#define TIME_OUT_CARD				            20					//检测出牌
#define TIME_ACTION				                20					//检测动作
#define TIME_SEND_CARD				            10					//检测摸牌
#define TIME_OFFLINE				            (5*60)					//检测离线
//网络补偿时间
...
{
	m_cbOutCardTime = TIME_OUT_CARD;								//出牌时间
	m_cbOperateTime = TIME_ACTION;								//操作时间
}
...
void CGameProcess::OnEventGameStart()
{
	...
	m_wCurrPlayCount++;
	//获得当前局数
	CT_WORD wCurrPlayCount = m_wCurrPlayCount;
	...
	for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
	{
		...
		//发送数据
		SendTableData(i, FPF_SUB_S_GAME_START, &GameStart, sizeof(FPF_CMD_S_GameStart), false);
		//判断听牌
		EstimateTingCard(i);
	}
    ...
    {
        m_pGameDeskPtr->SetGameTimer(ID_TIME_OUT_CARD, ((m_cbOutCardTime+TIME_NETWORK_COMPENSATE) * 1000));
    }
}
...
void CGameProcess::OnEventGameEnd(CT_DWORD dwChairID, GameEndTag GETag)
{
    ...
    {
        m_pGameDeskPtr->SetGameTimer(ID_TIME_READY, (((m_cbQiHuCardCount*1.2)+TIME_READY+TIME_NETWORK_COMPENSATE) * 1000));
...
void CGameProcess::OnEventGameScene(CT_DWORD dwChairID, CT_BOOL bIsLookUser)
{
	...
	case GAME_STATUS_FREE:	//空闲状态
	{
		...
		GameFree.wCurrPlayCount = m_wCurrPlayCount;
		//发送数据
	...
	case GAME_STATUS_PLAY:	//游戏中状态
	{
		...
		GamePlay.wCurrPlayCount = m_wCurrPlayCount;
		...
		memcpy(GamePlay.iTotalHuXi, m_iTotalHuXi, sizeof(GamePlay.iTotalHuXi));
	...
	case GAME_STATUS_END:					//游戏结束
	{
		...
		GameEnd.wCurrPlayCount = m_wCurrPlayCount;

		//剩余时间
		CT_DWORD dwPassTime = (CT_DWORD)time(NULL) - m_dwSysTime;
		CT_BYTE cbGameTime = TIME_READY;
		GameEnd.cbTimeLeave = (CT_BYTE)(cbGameTime - std::min<CT_BYTE>(dwPassTime, cbGameTime));

		GameEnd.dwCellScore = m_dwlCellScore;																		//基础积分
		...
		memcpy(GameEnd.iTotalHuXi, m_iTotalHuXi, sizeof(GameEnd.iTotalHuXi));
		//发送数据
...
void  CGameProcess::OnTimerMessage(CT_DWORD dwTimerID, CT_DWORD dwParam)
{
	...
	case ID_TIME_ACTION:				//动作定时器 
	{
        ...
        CT_DWORD dwActionNumber = m_dwActionNumber;
		//用户操作 
		for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
		{//是否有动作
			...
			//用户操作
			OnAutoOperateCard(i);
            if(dwActionNumber != m_dwActionNumber)
            {
                break;
            }
		}
	...
	case ID_TIME_READY:
    {
        m_pGameDeskPtr->KillGameTimer(ID_TIME_READY);
        if (m_cbGameStatus != GAME_STATUS_END) break;

        for (int i = 0; i < GAME_PLAYER; ++i)
        {
            if(sReady != m_pGameDeskPtr->GetUserStatus(i))
            {
    ...
    case ID_TIME_OFFLINE:
    {
		m_pGameDeskPtr->KillGameTimer(ID_TIME_OFFLINE);
        if(0 == m_wCurrPlayCount)
        {
            return;
        }
        //写分
        for (CT_DWORD i = 0; i < GAME_PLAYER; ++i)
        {
            ...
            CT_INT32 iRevenue[GAME_PLAYER] = {0};
            m_RecordScoreInfo[i].iScore = m_llTotalLWScore[i];
            //计算税收
            if (m_llTotalLWScore[i] > 0)
            {
                iRevenue[i] = (CT_INT32)m_pGameDeskPtr->CalculateRevenue(i, m_llTotalLWScore[i]);
                m_RecordScoreInfo[i].dwRevenue = iRevenue[i];
            }
            /*ScoreInfo  ScoreData;
            memset(&ScoreData, 0, sizeof(ScoreInfo));
            ScoreData.dwUserID = i;
            ScoreData.bBroadcast = true;
            ScoreData.llScore = m_llTotalLWScore[i]-iRevenue[i];
            ScoreData.llRealScore= m_llTotalLWScore[i]-iRevenue[i];
...
CT_BOOL  CGameProcess::OnGameMessage(CT_WORD wChairID, CT_DWORD dwSubCmdID, const CT_VOID* pDataBuffer, CT_DWORD dwDataSize)
{
	...
	case FPF_SUB_C_QI_HU:			//玩家弃胡
	{
		...
		SendTableData(INVALID_CHAIR, FPF_SUB_S_USER_QI_HU, &QiHu, sizeof(FPF_CMD_S_UserQiHu), false);

        CT_BYTE cbQiHuCount = 0;
        for (int j = 0; j < GAME_PLAYER; ++j)
        {
            if(m_cbUserQiHu[j])
            {
        ...
        if(cbQiHuCount == GAME_PLAYER)
        {
            m_cbQiHuCardCount = m_cbLeftCardCount;
        }
...
void CGameProcess::OnUserLeft(CT_DWORD dwChairID, CT_BOOL bIsLookUser, CT_BYTE byLeftTag)
{
    ...
    if(iCount >= GAME_PLAYER)
    {
        m_pGameDeskPtr->SetGameTimer(ID_TIME_OFFLINE, ((TIME_OFFLINE) * 1000));
    }
}
//清理游戏数据
...
CT_INT32 CGameProcess::GetGameRoundTotalScore(CT_DWORD dwChairID)
{
	...
	LOG_IF(INFO, PRINT_LOG_INFO) << "GetGameRoundTotalScore: Score=" << m_llTotalLWScore[dwChairID];
	return m_llTotalLWScore[dwChairID];
}
...
CT_INT32 CGameProcess::OnUserOperate(CT_DWORD dwChairID, FPF_CMD_C_OperateCard* pOperateData, CT_BOOL bTimeout)
{
	...
	{//发牌后动作 (翻牌后 没有偎 放弃胡牌)
		...
		if (EstimateSendCardPao(m_dwProvideUser, m_cbProvideCard, dwPaoUser, false))
		{
			...
			//发送数据
			SendTableData(INVALID_CHAIR, FPF_SUB_S_OPERATE_RESULT, &OperateResult, sizeof(FPF_CMD_S_OperateResult), false);
	...
	{//有人操作
		...
		{//发送出牌提示
			...
			memset(m_stUserAction, 0, sizeof(m_stUserAction));
			memset(m_stUserOperate, 0, sizeof(m_stUserOperate));
			LOG_IF(INFO, PRINT_LOG_INFO) << "OnUserOperate: 66666!  dwOperateUser= " << OperateResult.dwOperateUser << "m_dwCurrentUser= "<<m_dwCurrentUser;
			return 16;
		}
		return 17;
	}
...
CT_INT32 CGameProcess::OnUserOutCard(CT_DWORD dwChairID, CT_BYTE cbOutCard, CT_BOOL bTimeout)
{
	...
	for (CT_BYTE i = 0; i < GAME_PLAYER; ++i)
	{
		...
		if(bAction && dwPaoCardUser == INVALID_CHAIR)
        {
            OutCard.cbHasOperate = 1;
        }
		//提牌扑克列表
		...
		//发送数据
		SendTableData(i, FPF_SUB_S_OUT_CARD, &OutCard, sizeof(FPF_CMD_S_OutCard), /*(i == 0 ? true : false)*/false);
	}

	if(bAction && dwPaoCardUser == INVALID_CHAIR && (dwSendCardUser == INVALID_CHAIR))
    {
	    //有动作但不是跑
        m_pGameDeskPtr->SetGameTimer(ID_TIME_ACTION, ((m_cbOperateTime+TIME_NETWORK_COMPENSATE) * 1000));
    }
	else if(dwPaoCardUser != INVALID_CHAIR && dwSendCardUser == INVALID_CHAIR)
    {
	    //跑但是不是重跑
        m_pGameDeskPtr->SetGameTimer(ID_TIME_OUT_CARD, ((m_cbOutCardTime+TIME_NETWORK_COMPENSATE) * 1000));
    }
...
CT_BOOL CGameProcess::SendUserCard(CT_DWORD dwChairID)
{
	...
	for (CT_BYTE i = 0; i < GAME_PLAYER; ++i)
	{
		...
		{//有动作
			...
			LOG_IF(INFO, PRINT_LOG_INFO) << "SendCardAction: User=" << (int)i << ",Action=[" << (CT_WORD)SendCard.cbActionCode[ACK_CHI] << "," << (CT_WORD)SendCard.cbActionCode[ACK_PENG] << ","\
        ...
        if((bAction) && (!bTiCard) && (!bWeiCard) && (!bPaoCard))
        {
            SendCard.cbHasOperate = 1;
        }
		//发送数据
		SendTableData(i, FPF_SUB_S_SEND_CARD, &SendCard, sizeof(FPF_CMD_S_SendCard), /*(i == 0 ? true : false)*/false);
    ...
    if(bAction && (!bTiCard) && (!bWeiCard) && (!bPaoCard) && (dwSendCardUser == INVALID_CHAIR))
    {
        m_pGameDeskPtr->SetGameTimer(ID_TIME_ACTION, ((m_cbOperateTime+TIME_NETWORK_COMPENSATE) * 1000));
    }
    else if((dwSendCardUser == INVALID_CHAIR) && (bTiCard || bWeiCard || bPaoCard))
    {
        m_pGameDeskPtr->SetGameTimer(ID_TIME_OUT_CARD, ((m_cbOutCardTime+TIME_NETWORK_COMPENSATE) * 1000));
    }
	//判断听牌
...
CT_VOID CGameProcess::NormalGameEnd()
{
	...
	GameEnd.cbTimeLeave = TIME_READY;
	//发送数据
	SendTableData(INVALID_CHAIR, FPF_SUB_S_GAME_END, &GameEnd, sizeof(FPF_CMD_S_GameEnd), false);
...
CT_VOID CGameProcess::FlowBureauEnd()
{
	...
	{//私人场 
		//荒庄 庄家扣10分
        GameEnd.iLWHuXi[m_dwBankerUser] -= 10;
...
CT_BOOL CGameProcess::OnAutoOperateCard(CT_DWORD dwChairID)
{
    ...
    OperateCard.dwUserID = m_pGameDeskPtr->GetUserID(dwChairID);
	OperateCard.dwOperateCode = ACK_NULL;
    OperateCard.dwActionNumber = m_dwActionNumber;

    if(m_stUserAction[dwChairID].cbActionCode[ACK_CHIHU])
    {
        OperateCard.dwOperateCode = ACK_CHIHU;
        OperateCard.cbOperateCard = m_stUserAction[dwChairID].cbActionCard;
    }
...
CT_BOOL CGameProcess::GetChiCard(CT_DWORD dwChairID, CT_BYTE cbCurrentCard, CT_BYTE cbChiWeaveCard[MAX_ITEM_COUNT][3], CT_BYTE& cbChiWeaveCount, CT_BYTE& cbMustIndex)
{
	...
	memset(WeaveCard, 0, sizeof(WeaveCard));
	...
	memset(cbSuccWeave, 0, sizeof(cbSuccWeave));
	...
	memset(cbFindWeave, 0, sizeof(cbFindWeave));
	for (CT_WORD i = 0; i < 3; ++i)
	{
...
CT_VOID CGameProcess::SendTotalClearing(std::string strTime)
{
	...
	for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
	{
		...
		ClearingData.dTotalLWScore[i] = m_llTotalLWScore[i]*TO_DOUBLE;				//各玩家总输赢分
	}
...
{
    CT_INT32 iTotalHuXi[GAME_PLAYER] = {0};
	//计算输赢
	for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
	{
	    if(m_iTotalHuXi[i] > MAX_HUXI)
        {
            m_iTotalHuXi[i] = MAX_HUXI;
        }
		//个位数四舍五入
	...
	//计算总输赢
	CT_LONGLONG iTempScore[GAME_PLAYER][GAME_PLAYER];
	memset(iTempScore, 0, sizeof(iTempScore));
	CT_LONGLONG llTempTotalScore[GAME_PLAYER]={0};
    CT_LONGLONG llMaxWinLoseScore[GAME_PLAYER] = {0};
	for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
	{
        llMaxWinLoseScore[i] = m_pGameDeskPtr->GetUserScore(i);
		for (CT_WORD j = 0; j < GAME_PLAYER; ++j)
		{
			if (j == i) continue;
			iTempScore[i][j] = iTotalHuXi[i] - iTotalHuXi[j];
			iTempScore[i][j] *= m_dwlCellScore;
			llTempTotalScore[i] += iTempScore[i][j];
		}
	}
	CT_WORD wMaxDiff = 0;
    CT_WORD wMaxDiffChairID = INVALID_CHAIR;
    for (CT_WORD l = 0; l < GAME_PLAYER; ++l)
    {
        if(llTempTotalScore[l] > 0 && ((llMaxWinLoseScore[l] - llTempTotalScore[l]) < 0) && abs(llMaxWinLoseScore[l] - llTempTotalScore[l]) > wMaxDiff)
        {
            wMaxDiff = abs(llMaxWinLoseScore[l] - llTempTotalScore[l]);
            wMaxDiffChairID = l;
        }
        if(llTempTotalScore[l] < 0 && ((llMaxWinLoseScore[l] + llTempTotalScore[l]) < 0) && (abs(llMaxWinLoseScore[l] + llTempTotalScore[l]) > wMaxDiff))
        {
	...
	//判断输赢上下限不能超过自己随身携带
    if(wMaxDiffChairID != INVALID_CHAIR)
    {
        CT_DOUBLE dRatio = fabs((CT_DOUBLE)llMaxWinLoseScore[wMaxDiffChairID]/(CT_DOUBLE)llTempTotalScore[wMaxDiffChairID]);
        for (CT_WORD i = 0; i < GAME_PLAYER; ++i)
        {
            if(i == wMaxDiffChairID)
            {
                if(llTempTotalScore[i] > 0)
                {
                    m_llTotalLWScore[i] = llMaxWinLoseScore[i];
                }
                else
                {
                    m_llTotalLWScore[i] = -llMaxWinLoseScore[i];
            ...
            m_llTotalLWScore[i] = dRatio*llTempTotalScore[i];
        }
    }
    else
	{
		memcpy(m_llTotalLWScore, llTempTotalScore, sizeof(m_llTotalLWScore));
	}

    LOG_IF(INFO, PRINT_LOG_INFO) << "TotalHuXi[0]= " << m_iTotalHuXi[0] << ", TotalHuXi[1]= "<<m_iTotalHuXi[1]<<", TotalHuXi[2]= "<< m_iTotalHuXi[2];
    LOG_IF(INFO, PRINT_LOG_INFO) << "llTempTotalScore[0]= " << llTempTotalScore[0] << ", llTempTotalScore[1]= "<<llTempTotalScore[1]<<", llTempTotalScore[2]= "<< llTempTotalScore[2];
    LOG_IF(INFO, PRINT_LOG_INFO) << "SourceScore[0]= " << llMaxWinLoseScore[0] << ", SourceScore[1]= "<<llMaxWinLoseScore[1]<<", SourceScore[2]= "<< llMaxWinLoseScore[2];
    LOG_IF(INFO, PRINT_LOG_INFO) << "TotalLWScore[0]= " << m_llTotalLWScore[0] << ", TotalLWScore[1]= " <<m_llTotalLWScore[1] <<", TotalLWScore[2]= "<< m_llTotalLWScore[2];
	//写分
	for (CT_DWORD i = 0; i < GAME_PLAYER; ++i)
	{
		m_RecordScoreInfo[i].dwUserID = m_pGameDeskPtr->GetUserID(i);
		...
		if (m_llTotalLWScore[i] > 0)
		{
			iRevenue[i] = (CT_INT32)m_pGameDeskPtr->CalculateRevenue(i, m_llTotalLWScore[i]);
			m_RecordScoreInfo[i].dwRevenue = iRevenue[i];
            m_llTotalLWScore[i] -= iRevenue[i];
		}
		ScoreInfo  ScoreData;
		memset(&ScoreData, 0, sizeof(ScoreInfo));
		ScoreData.dwUserID = i;
		ScoreData.bBroadcast = true;
		ScoreData.llScore = m_llTotalLWScore[i];
		ScoreData.llRealScore= m_llTotalLWScore[i];
		m_pGameDeskPtr->WriteUserScore(i, ScoreData);
	}
	m_pGameDeskPtr->RecordRawInfo(m_RecordScoreInfo, GAME_PLAYER, NULL, 0, 0, 0, 0, 0, 0, 0);
}
